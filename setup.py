...
# so gh_doc_automation __init__ can detect if it's being loaded in the setup
...
# gh_doc_automation to build the compiled extensions in sub-packages
builtins.__GH_DOC_AUTOMATION_SETUP__ = True

# metadata
DISTNAME = 'gh_doc_automation'
PYPIDIST = DISTNAME
...
import gh_doc_automation
VERSION = gh_doc_automation.__version__
...
if SETUPTOOLS_COMMANDS.intersection(sys.argv):
    ...
    if any(cmd in sys.argv for cmd in set(['develop', 'bdist_wheel',
                                           'bdist_wininst'])):
        we_be_buildin = True
...
class CleanCommand(clean):
    ...
    # this is mostly in case we ever add a Cython module to gh_doc_automation
    def run(self):
        clean.run(self)
